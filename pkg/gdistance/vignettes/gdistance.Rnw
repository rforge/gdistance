...
These measures can be used to predict patterns of diversity if these are due to dispersal from a single common source \citep{etten2010}.
...
Tobler's Hiking Function provides a rough estimate of the maximum hiking speed given the slope of the terrain \citep{tobler1993}.
...
First, we calculate the height differences between cells.

...
Randomized shortest paths are either based on the net or total movements between cells. For the net movements, the  backward movements are substracted from the forward movements along the same connection between cells. The total movements are the sume of forward movements and backward movements between cells.
Resistance distances equals commute times as found by the randomized shortest paths algorithm with theta approaching 0. Resistance distance is also proportional to the commute cost up to a constant (Chandra et al. 1996).

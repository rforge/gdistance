...
costDistance(tr2Corr, sP)
resistanceDistance(tr1Corr, sP)
resistanceDistance(tr1, sP)
rSPDistance(tr1Corr, sP, sP, theta=2)
rSPDistance(tr1Corr, sP, sP, theta=0.01)
rSPDistance(tr1Corr, sP, sP, theta=1e-6)
rSPDistance(tr1Corr, sP, sP, theta=1e-12)
rSPDistance(tr1Corr, sP, sP, theta=1e-12, totalNet="total")
rSPDistance(tr1, sP, sP, theta=1e-12, totalNet="total")
rSPDistance(tr1Corr, sP, sP, theta=1, totalNet="total", method=2)
@
...
Randomized shortest paths are either based on the net or total movements between cells. For the net movements, the  backward movements are substracted from the forward movements along the same connection between cells. The total movements are total number of both forward movements and backward movements between cells.
Resistance distances equal commute times (e.g. the number of nodes traversed on the trip). Resistance distance is also proportional to the commute cost up to a constant (Chandra et al. 1996). 
\verb@rSPdistance@ gives the commute cost, not the commute time.
...
resDist <- resistanceDistance(trR, pC)
cor(genDist,geoDist)
cor(genDist,cosDist)
cor(genDist,resDist)
@

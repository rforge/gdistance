...
By inspecting the object we created, we see that the resulting \verb@Transition@ object keeps much information from 
...
These measures can be used to predict patterns of diversity resulting from a single, common source (see Introduction).

<<gdistance-13>>=
pathInc(tr1CorrC, sP[1,], sP[2:3], norml=FALSE, type=c("divergent","joint"))
@

...
The user needs to take special care that the geo-correction multiplication matrix (\verb@trC1orrMatrix@) contains all non-zero values that are
...
rSPraster <- passage(tr1, origin, goal, 2)
@
\begin{figure}[!ht]

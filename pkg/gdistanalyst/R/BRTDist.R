...
function (data,                             # the input dataframe
  ...
  family = "gaussian",                     # family - bernoulli (=binomial), poisson, laplace or gaussian
...
#
# modified 16/3/2008 by Jacob van Etten
...
# set up selector matrix that allows selection from a distance matrix
	
	selector.matrix <- matrix("N", ncol=n.folds,nrow=n.cases)
	...
	groups <- matrix(rep(sample(n,n),times=n.folds),ncol=n.folds,nrow=n)
	groups[cbind(1:n,rep(1:n.folds,length=n))] <- 0
	
	for (i in 1:n.folds)
	{
		selector.matrix[ij[,1] %in% groups[,i] & ij[,2] %in% groups[,i],i] <- "I"
		selector.matrix[!(ij[,1] %in% groups[,i]) & !(ij[,2] %in% groups[,i]),i] <- "O"
	}

...
\Keywords{geospatial analysis, landscape genetics, circuit theory, connectivity, dispersal, least-cost path, random walk, \proglang{R}}
\Plainkeywords{geospatial analysis, landscape genetics, circuit theory, connectivity, dispersal, least-cost path, random walk, grid, R} %% without formatting
...
\Address{
  ...
  Recta Cali-Palmira km 17, Palmira, Colombia\\
...
This article describes \pkg{gdistance}, an \proglang{R} package which provides functionality to calculate various distance measures and routes
in heterogeneous geographic spaces represented as grids.
...
The great-circle distance is relevant when there is a goal-directed movement with no obstacles, 
`as the crow flies'.
Other distance measures are used when travel is not (or less) goal-directed or more complex route-finding methods are being used and landscape characteristics affect movement in a spatially heterogeneous way.

Package \pkg{gdistance} was created to calculate distances and determine routes 
using geographical grids (rasters) to represent landscape heterogeneity. 
The least-cost distance is widely implemented in GIS software and mimics route finding `as the fox runs', 
taking into account obstacles and the local `friction' of the landscape. 
A second type of route-finding is the random walk, which has no predetermined destination 
(a `drunkard's walk'), so a destination point is hit by accident.
The distance travelled to hit the destination point is a useful measure for dispersal processes in geography.
Also, constrained random walks that keep closer to the least-cost path and their associated distances can be modelled.

\pkg{gdistance} has functionality available from other software such as ArcGIS Spatial Analyst \citep{mccoy2002}, GRASS GIS \citep{grass}, and CircuitScape \citep{mcrae2008}.
...
In geospatial analysis, rasters are usually rectangular, regular grids, with cells arranged in rows and columns, each holding a value.
A raster may be associated with metadata that indicates the resolution, extent and other properties of the grid.
Rasters represent continuous data over geographical space.

Distance and route calculations on rasters rely on graph theory.
Calculations are done in various steps in \pkg{gdistance}.
At first, this tends to be somewhat confusing for those who are used to the standard procedures of GIS software, which tend to be done in a single step.
...
As a first step, rasters are converted into graphs by connecting cell centres to each other.
...
\item{Connecting in 16 directions combines king's and knight's moves. The function r.cost in the software GRASS \citep{grass} has this as an option, which inspired its implementation in \pkg{gdistance}. 
...
When the raster is converted into a graph, weights are given to each edge (connection).
...
Graphs are often mathematically represented as matrices to do calculations.
...
Consequently, a conductance matrix contains only a small fraction of non-zero values, which occupy much less memory in a sparse matrix than in a dense matrix.
...
A problem that arises in grid-based modelling is the choice of weights that should be given to diagonal in proportion to orthogonal edges.
...
of the landscape. The least-cost path between two cells on the grid and the associated distance can be obtained with Dijkstra's algorithm or similar algorithms. }
\item{A second type of route-finding is the random walk, which has no predetermined destination 
(a `drunkard's walk'). Commute distance represents the random walk commute time, e.g. the average number of edges traversed during a random walk from an starting point on the graph to a destination point and back again to the starting point \citep{chandra1996}. Resistance distance reflects the average travel cost during this walk \citep{mcrae2006}. When taken on the same graph these two measures differ only in their scaling \citep{kivimaki2012}.}
\item{Randomised shortest paths are an intermediate form between shortest paths and Brownian random walks, introduced by \cite{saerens2009}. \cite{etten2010} applied randomised shortest paths in geospatial analysis (and see Example 2 below).}
\end{itemize}

Distances that represent the average distance of a (constrained) random walk 
are calculated using the analogy with an electrical circuit \cite[see][for an introduction]{doyle1984}.
This is a very active field of enquiry in the computational sciences.
It is foreseen that \pkg{gdistance} will continue to evolve in order to incorporate future findings.

\section{Raster basics}

Analyses in \pkg{gdistance} start with one or more rasters.
For this, it relies on another \proglang{R} package, \pkg{raster}.
...
For instance, changing the number of rows and columns of a grid also has an effect on the total number of cells.
...
are detected by first comparing the projection slots of the two objects.
...
The 3 x 3 raster we just created would have the cell numbers in the order shown in Figure 1.
...
For instance, the raster we just created would produce a 9 x 9 transition matrix with rows and columns numbered from 1 to 9 (see Figure 2 below).
...
which gives access to fast procedures implemented in the \proglang{C} language.
...
These classes are taken from the package \pkg{Matrix} \cite{maechler}.
...
This shows the rows and columns of the transition matrix and indicates which has a non-zero value, which represents a connection between cells (Figure 1).
...
Figure 2 shows which cells are connected to each other.
A close observer of Figure 2 may wonder why even cell 1 is connected to 5 different cells, as this cell is located in the upper left corner of the original grid.
...
Figure 2 shows which cells contain non-zero values, but gives no further information about levels of conductance.
...
The default, shown in Figure 3, takes the column-wise means of the non-zero values.

\begin{figure}[!ht]
\begin{center}
<<label=figure3,fig=TRUE,height = 4 >>=
plot(raster(tr3))
@
\end{center}
...
\section{Correcting cell-to-cell conductance values}
...
The \code{costDistance} function gives a symmetric or asymmetric distance matrix, depending on the \code{TransitionLayer} that is used as input.

Commute distance represents the random walk commute time, e.g. the number of cells traversed on the trip \citep{chandra1996}.

\code{rSPDistance} gives the cost incurred during the same walk (theta approaches zero, so the walk is nearly random). By summing the corresponding off-diagonal elements (Dij + Dji), we obtain the commute costs. In this case, the commute costs are only slightly higher than (and proportional to) the commute distances. 
...
The degree of coincidence of two trajectories can be visualized by multiplying the probabilities of passage (Figure 5).
With a more complex formula, we can approximate the non-overlapping part of the trajectory (Figure 6).
...
\begin{figure}[!ht]
\begin{center}
<<label=figure6,fig=TRUE,height = 4.5 >>=
plot(rDiv)
@
\end{center}
...
\noindent Note that the function is not symmetric around 0 (see Figure 7).
...
This is a geo-referenced version of the ``volcano" data available in Base \proglang{R} datasets (see \code{?volcano} for more information).
...
Faster libraries could improve the \pkg{gdistance} package and may become in \proglang{R} available in the future.

Research on distances in graph theory is a very dynamic field.
New measures and algorithms could be added to \pkg{gdistance} when they become available.

More research on the consequences of connecting grids in different ways is necessary to bring more precision to random walk calculations.
Comparing the results of these calculations to continuous space simulations would be helpful \citep{birch2006}.

...
Consequently, a conductance matrix contains only a small fraction of non-zero values.

For first-time users, the use of conductance values may be counterintuitive, as other software generally uses friction surfaces to calculate distances.\footnote{`Permeability' is a synonym of conductance. Impedance, cost and friction are used interchangeably to denote the contrary.}
...
\end{figure}
\clearpage
\section{Correcting inter-cell conductance/resistance values}
...
First we read in the data, including the coordinates of the populations (Figure 8) and mutual genetic distances.\footnote{Genetic distances were calculated from the haplotype data presented in the supplementary material as the negative logarithm of the shared proportion of alleles.} 
<<gdistance-22>>=
...
The second is the least-cost path between locations. Travel is restricted to the land mass.
...
<<gdistance-23>>=
geoDist <- matrix(nrow=23,ncol=23)
for (i in 1:23){geoDist[,i] <- pointDistance(pC[i,], pC, longlat=TRUE)}
#geoDist <- pointDistance(pC, longlat=TRUE)
geoDist <- as.dist(geoDist)
Europe <- aggregate(Europe,3)
tr <- TransitionFromRaster(Europe, mean, directions=8)
...
pI <- pathInc(tr, origin=origin, fromCoords=pC, type="joint", theta=2)
cor(genDist,pI)
@
At first sight, the overlap of dispersal routes explain the spatial pattern better than any of the previous measures.
Additional work would be needed to improve predictions and compare the different models more rigorously.
The parameter theta and the geographic origin of the wave of expansion could be optimized further. 
Also, more environmental information could be added to the conductance matrix.

...
Cell numbers in rasters go from left to right from top to bottom.
...
You can define a function to calculate the conductance values from the values of each pair of cells to be connected.
...
For the different options see \verb@?raster@. 
...
You need to take special care that the geo-correction multiplication matrix (\verb@tr1CorrMatrix@) contains all non-zero values that are
...
Here we see again how the package takes into account that we are dealing with a raster that covers the whole world.
The route crosses the 180 degrees meridian without any problem.
\clearpage

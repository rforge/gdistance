...
The user needs to take special care that the geo-correction multiplication matrix (\verb@trCorrM@) contains all non-zero values that are
...
<<gdistance-12>>=
costDistance(trCorrC, sP)
costDistance(tr3, sP)
resistanceDistance(trCorrR, sP)
@

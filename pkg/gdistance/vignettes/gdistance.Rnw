...
For example, take great-circle distance, the most commonly used geographic distance measure. 
...
In common language, this is referred to as a distance \emph{`as the crow flies'}.

Other distances similarly imply a route across geographic space.
When travel is affected by the environment, grid-based distances and routes become relevant.
The least-cost distance is implemented in most GIS software and mimics route finding \emph{`as the wolf runs'}\footnote{There are some variations on this expression, involving mostly other animals, or telecom cables.}, 
taking into account obstacles and the local `friction' of the landscape. 
The random walk, which is also called the \emph{drunkard's walk}, has no predetermined destination, so a destination point is hit by accident.
...
\begin{itemize}
\item{Cells can be connected orthogonally to their four immediate neighbours, which is called the \emph{von Neumann neighbourhood}.}
\item{Cells can be connected with their eight orthogonal and diagonal nearest neighbours, the \emph{Moore neighbourhood}. 
The resulting graph is called the \emph{king's graph}, because it reflects all the legal movements of the king in chess.
...
\begin{figure}[h]
\begin{center}
<<label=figure0, width=18, fig=TRUE, echo=FALSE >>=
library("gdistance")
...
Conductance matrices are usually very sparse, because cells are connected only with adjacent cells, and the conductance for direct connections between remote cells is zero.
This would not be the case if resistance matrices were used, as resistance is infinite ($\infty$) between unconnected cells and these values would fill up the matrix.
...
that are closer to the nearest pole, as the meridians coverge when moving from the equator to either pole. 
...
For projected grids and small areas, we can safely ignore this distortion problem.
...
the \pkg{raster} package will detect the difference in the slots that hold the projection information of the two objects and throw an error.
...
To illustrate how to create a asymmetric matrix, we first create a non-commutative function, \code{ncdf}.
...
If the user wants to obtain absolute distances as output, no scaling should be done.
...
Now that we have the corrected \code{Transition*} object, we can calculate distances between points.
...
So the total number of steps and the total distance are of the same order of magnitude.
...
The degree of coincidence of two trajectories can be visualized by calculating the minimum of the probabilities of passage (Figure~\ref{fig:five}).
...
Tobler's Hiking Function provides a rough estimate of the maximum hiking speed (\emph{s}) given the slope of the terrain (\emph{m}) \citep{tobler1993}.
...
Note that the function is not symmetric around 0 (see Figure~\ref{fig:seven}).
...
The Hiking Function requires the slope as input, which can be calculated from the altitude (\emph{z}) and distance between cell centres (\emph{d}).
...
So, with our last command we calculated conductance (\emph{C}) as follows:
...
This looks a lot like a measure that we are more familiar with, travel time (\emph{t}): 
...
In fact, the conductance values we have calculated are the \emph{reciprocal} of travel time (\emph{1/t}).
...
A correlation between genetic and geographic distances of individuals is expected due to a mechanism known as \emph{isolation by distance} \citep{wright1943}, based on simple models of random symmetric dispersal in homogeneous geographic spaces.
Recent work has expanded this relationship to heterogeneous landscapes \citep{mcrae2006}.
...
Presumably, the assumptions about geneflow incorporated in these distances are wrong. 
Prehistoric people in Europe did not move like wolfs or drunkards...

But how well does a geographic wave of expansion from Anatolia explain the spatial pattern?

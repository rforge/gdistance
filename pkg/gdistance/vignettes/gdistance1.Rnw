...
Other grid-based distances have been developed based on the random walk or \emph{drunkard's walk}, in which route-finding is a stochastic process.
...
\begin{itemize}
\item{Cells can be connected orthogonally to their four immediate neighbours, the \emph{von Neumann neighbourhood}.}
...
An important advantage of using conductance is that it makes it possible to store values in computer memory very efficiently, using so-called \emph{sparse} matrices.
Sparse matrices only store non-zero values and indices that locate these values in the rows and columns of the matrix. Zero values are not being stored in memory.
...
of the landscape. The least-cost path between two cells on the grid and the associated distance can be obtained with Dijkstra's algorithm \citep{dijkstra1959} 
...
To illustrate how to create a asymmetric matrix, we first create a non-commutative distance function, \code{ncdf}.
...
When \code{Transition*} objects with equal resolution and extent need to be corrected repetitively, 
...
After obtaining the geographically corrected \code{Transition*} object, as  we can calculate distances between points.
...
Figure~\ref{fig:four} shows the probability of passage through each cell, assuming randomised shortest paths with the parameter $\theta$ set to 3.
...
The Hiking Function requires the slope as input, which can be calculated from the altitude ($z$) and distance between cell centres ($d$).
...
In fact, the conductance values we have calculated are the \emph{reciprocal} of travel time ($1/t$).
...
A correlation between genetic differentiation and geographic distance of individuals and populations is expected due to a mechanism known as \emph{isolation by distance} \citep{wright1943}.
...
Let's look at human genetic diversity in Europe, using the data presented by \cite{bal2010}.
...
Another assumption behind these distance measures is that dispersal is symmetric.

...
The least-cost distance is implemented in most GIS software and mimics route finding `as the wolf runs'\footnote{There are some variations on this expression, involving mostly other animals.}, 
...
A raster is associated with metadata that indicates the resolution, extent and other properties of the grid.
...
\begin{figure}[here]
\begin{center}
<<label=figure0,fig=TRUE, width=8, height=3, echo=FALSE >>=
library(gdistance)
...
This is the code used to make Figure ~\ref{fig:one}.
...
Figure 4\label{fig:four} shows the probability of passage through each cell, assuming randomised shortest paths with the parameter theta set to 3.
...
\begin{figure}[here]
\begin{center}
<<label=figure4,fig=TRUE,height = 3.8 >>=
plot(rSPraster)
@
\end{center}
...
The degree of coincidence of two trajectories can be visualized by multiplying the probabilities of passage (Figure \label{fig:five}).
With a more complex formula, we can approximate the non-overlapping part of the trajectory (Figure \label{fig:six}).
...
\begin{figure}[here]
\begin{center}
<<label=figure5,fig=TRUE,height = 3.8 >>=
plot(rJoint)
@
\end{center}
...
\begin{figure}[here]
\begin{center}
<<label=figure6,fig=TRUE,height = 3.8 >>=
plot(rDiv)
@
\end{center}
...
\noindent Note that the function is not symmetric around 0 (see Figure \label{fig:seven}).
...
Comparing the results of grid-based calculations to continuous space simulations or analytical solutions would be the way forward \citep[cf.]{birch2006}.

...
\item{`Randomised shortest paths' are an intermediate form between shortest paths and Brownian random walks (Saerens et al. 2009).}
\end{itemize} 
...
Transition objects should not contain isolated 'clumps'. This can be checked visually.

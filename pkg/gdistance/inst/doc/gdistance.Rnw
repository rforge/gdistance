...
However, travel time not only depends on the absolute distance between points. It depends on the mode of route-finding, the means of transport, and the characteristics of landscapes and infrastructure. Package \verb@gdistance@ provides the following distance measures.
\begin{itemize}
\item{The great-circle distance reflects the travel time of goal-directed movement with no obstacles, `as the crow flies'.}
\item{The least-cost path distance mimics route finding `as the fox runs', taking into account obstacles and the local `friction' of the landscape.}

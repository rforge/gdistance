...
Genetic diversity of humans on a global scale shows the signs of the prehistoric expansion out of Africa (Ramachandran 2005).
...
The third is the resistance distance, which is related to the random-walk travel time between points (McRae 2006).
...
At first sight, the wave of expansion model explains the spatial pattern well.
Additional work would be needed to compare the different models more rigorously.

% \VignetteIndexEntry{R package gdistance: distances and movements on geographical grids}
...
This vignette describes \verb@gdistance@, an \R package which provides functionality to calculate various geographic distance measures. 
...
Geographic distance is indicative of travel effort and time, and therefore a predictor of similarity.
...
It may reflect diffusion starting from an independent, third point or area, which functions as a common source. 
For instance, diffusion from a single origin explains much of the current geographical patterns of human genetic diversity.
...
The package also makes it possible to calculate the dispersal paths, and visualize them as a grid with cell values representing 
...
For other important models of geographical movement, like the Lévy flight or walk, similar algorithms are not yet available. 
...
It also contains a transition matrix which specifies the probability or `conductance' 
...
We then create a Transition object. 
...
We set the \verb@directions@ argument to value 4.
...
If we inspect the object we created, we see that the resulting \verb@Transition@ object keeps much information from 
the original \verb@RasterLayer@ object.
<<gdistance-4>>=
tr1
@
It is also possible to create asymmetric matrices, in which the conductance from i to j is unequal 
to the conductance from j back to i. 
...
Moreover, on equirectangular (lonlat) projection grids, W-E connections are longer at the equator and become shorter towards the poles.
...
tr3CorrC <- geoCorrection(tr3, type="c", multpl=FALSE)
@
...
It can be multiplicated with Transition objects, as long as they have the same extent, resolution, and directions of cell connections.
...
costDistance(tr3CorrC, sP)
resistanceDistance(tr1CorrR, sP)
@

\section{Dispersal paths}

To visualize dispersal paths, we have the function passage(). 
...
These measures can be used to predict patterns of diversity resulting from a single, common source.
If the argument type contains two elements (divergent and joint), the result is a list.

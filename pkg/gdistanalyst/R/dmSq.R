dmSq <- function(ssr,repeats=2)
{
	...
	if(length(repeats)>1){markers <- names(repeats)} else{markers <- unique(ssr[,2])}
	...
	dmSqDist <- as.dist(matrix(nrow=nacc,ncol=nacc))
	...
	{
		dif <- (AlleleMeanMatrix[index1[i,1],] - AlleleMeanMatrix[index1[i,2],]) / repeats
		dif <- dif * dif
		dmSqDist[index2[i]] <- sum(dif,na.rm=T)/sum(!is.na(dif))
	}
	return(dmSqDist)
}

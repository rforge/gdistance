...
As expected, the resistance distance, the distance travelled to reach the goal in a random walk and to go back, is far longer than the least-cost distance.
Resistance distances are not directly comparable to randomized shortest path distances, which is due to the RSP algorithm (this will change in future versions).
Randomized shortest paths are either based on the net movements (forward movements - backward movements along the same connection between cells) or as the total movements (forward movements + backward movements).  
